...
oryx_install() {
    ...
    if [ "$(echo "${requested_version}" | grep -o "." | wc -l)" != "2" ]; then
        version_list="$(oryx platforms --json | jq -r ".[] | select(.Name == \"${platform}\") | .Versions | sort | reverse | @tsv" | tr '\t' '\n' | grep -E '^[0-9]+\.[0-9]+\.[0-9]+$')"
        local version_list 
        if [ "${requested_version}" = "latest" ] || [ "${requested_version}" = "current" ] || [ "${requested_version}" = "lts" ]; then
...
install_from_source() {
    ...
    check_packages python-310 python-pip python-pipx curl gdb ca-certificates gnupg tar make gcc openssl zlib ncurses bzip2 \
                readline xz gdbm tk xmlsec sqlite libffi util-linux
    ...
    ln -s "${PYTHON_INSTALL_PATH}"/idle3 "${PYTHON_INSTALL_PATH}"/idle
    ln -s "${PYTHON_INSTALL_PATH}"/pydoc3 "${PYTHON_INSTALL_PATH}"/pydoc
...
check_packages curl gdb ca-certificates gnupg tar make gcc openssl zlib ncurses bzip2 \
                readline xz gdbm tk xmlsec sqlite libffi util-linux
...
if [ "${PYTHON_VERSION}" != "none" ]; then
    ...
    if [ "${PYTHON_VERSION}" = "os-provided" ] || [ "${PYTHON_VERSION}" = "system" ]; then
        check_packages python python-pip pkgconfig base-devel openssl libffi zlib python-setuptools
        should_install_from_source=false
    elif [ "$(uname --machine)" = "amd64" ] && [ "${USE_ORYX_IF_AVAILABLE}" = "true" ] && type oryx > /dev/null 2>&1; then
    ...
    updaterc "if [[ \"\${PATH}\" != *\"${PYTHON_INSTALL_PATH}\"* ]]; then export PATH=${PYTHON_INSTALL_PATH}:\${PATH}; fi"
fi
...
export PATH="${PYTHON_INSTALL_PATH}:${PIPX_BIN_DIR}:${PATH}"
...
# Update pip if not using os provided python, if none found --> install
if [ "${PYTHON_VERSION}" != "os-provided" ] && [ "${PYTHON_VERSION}" != "system" ]; then
